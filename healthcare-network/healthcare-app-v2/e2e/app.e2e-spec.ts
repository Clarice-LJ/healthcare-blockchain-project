...
describe('Starting tests for healthcare-app-v2', function() {
  ...
  it('website title should be healthcare-app-v2', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('healthcare-app-v2');
    })
  });

  it('network-name should be healthcare-network@0.0.3',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('healthcare-network@0.0.3.bna');
    });
  });

  it('navbar-brand should be healthcare-app-v2',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('healthcare-app-v2');
    });
  });
    ...
    it('Record table should have 8 columns',() => {
      page.navigateTo('/Record');
    ...
    it('HealthcareProvider table should have 6 columns',() => {
      page.navigateTo('/HealthcareProvider');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(6); // Addition of 1 for 'Action' column
      });
    ...
    it('Government table should have 6 columns',() => {
      page.navigateTo('/Government');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(6); // Addition of 1 for 'Action' column
      });
